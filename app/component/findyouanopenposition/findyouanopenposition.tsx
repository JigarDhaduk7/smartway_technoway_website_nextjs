            ...
            {jobs.map((job, index) => (
              <div key={job._id} className="accordion-item">
                <h2 className="accordion-header">
                  <button className={`accordion-button font-14 pe-0 ${index !== 0 ? 'collapsed' : ''}`} type="button" data-bs-toggle="collapse" data-bs-target={`#accordion${index + 1}`} aria-expanded={index === 0} aria-controls={`accordion${index + 1}`}>
                    <div className='col-auto'>
                      <div className='text-secondary fw-semibold mb-1'>{job.title}</div>
                      <div className='text-primary-emphasis fw-normal'>{job.location}</div>
                    </div>
                    <div className='col'>
                ...
                <div id={`accordion${index + 1}`} className={`accordion-collapse collapse ${index === 0 ? 'show' : ''}`} data-bs-parent="#DefaultAccordionsWrap">
                  <div className="accordion-body">
                          ...
                          <div className='row justify-content-between'>
                            <div className='col-xl-6 col-xxl-5 mb-3'>
                              ...
                              {job.requiredSkills.map((skill, skillIndex) => (
                                <p key={skillIndex} className="peragraph-style-1 list-sty-arrow">
                                  <i className="far fa-angle-double-right text-secondary me-2"></i> {skill}
                                </p>
                              ))}
                            </div>
                            <div className='col-xl-6 col-xxl-5 mb-3'>
                              ...
                              {job.responsibilities.map((responsibility, respIndex) => (
                                <p key={respIndex} className="peragraph-style-1 list-sty-arrow">
                                  <i className="far fa-angle-double-right text-secondary me-2"></i> {responsibility}
                                </p>
                              ))}
                            </div>
                          </div>
                    ...
                    <div className='text-end mt-4 mb-1'>
                      <div className='d-inline-block'>
                        <a href="" className="btn btn-primary btn-lg d-flex align-items-center">
                          <i className="fas fa-envelope font-18 me-2"></i> Apply Now
                        </a>
                      </div>
                    </div>
                  </div>
